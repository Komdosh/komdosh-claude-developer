...
| Schema change / Liquibase changeset | `/add-migration` (core) |
| Kafka/SQS/RabbitMQ consumer | `core/event-consumer-author` |
| Avro schema + generated event DTOs | `/avro-new-event` → `core/avro-schema-author` |
| Spring config, profiles, feature flags | `core/backend-implementer`, per `core/rules/configuration.md` |
| Gradle / version catalog | `core/backend-implementer`, per `core/rules/gradle-build.md` |
| Docker Compose / Dockerfile | `core/backend-implementer`, per `core/rules/local-dev.md` |
| Kubernetes / ArgoCD / Terraform / CI | the `komdosh-dev-infra-*` plugins |
| Metrics, traces, structured logging | `core/backend-implementer`, per `core/rules/observability.md` |
| AuthN/AuthZ filters, protected routes | `core/backend-implementer`, per `core/rules/spring-security.md`; post-hoc audit via `/security-audit` (quality) |
| Tests (unit / integration / architecture) | `core/test-writer` |
| Vendor-decoupling / common module extraction | `/audit-leaks` → `quality/platform-developer` |
| QA artifacts (manual plan, Postman, console) | `/qa [plan\|postman\|console\|all]` (quality) |
| Captured architectural decision (within service) | `/adr-new` (core) |
| Cross-service / durable decision (ADR blocker) | the **architecture repo's** `new-adr` playbook — not a service-repo ADR |
| Release prep, changelog, rollback playbook | `/release-prep` `/changelog` `/rollback-playbook` (this plugin) |
