...
Plugins detected: <every installed komdosh-dev-* plugin and its version, as discovered in Step 1 —
                  not a fixed list; an absent plugin's gates are N/A, an undetectable one's are UNKNOWN>
...
| 17 | Release readiness     | N/A     | komdosh-dev-spring-delivery not installed |
| 18 | Changelog up to date  | N/A     | komdosh-dev-spring-delivery not installed |
| 19 | Rollback playbook     | N/A     | komdosh-dev-spring-delivery not installed |
...
**Discover the installed set — never hardcode a plugin list.** A hardcoded list silently goes stale the moment the marketplace ships a plugin, and every gate belonging to an unlisted plugin then disappears from the map without a word. Enumerate what is actually on disk instead:

```bash
# Every installed plugin whose name starts with the marketplace prefix, with its version.
for d in "$HOME/.claude/plugins" "$HOME/.claude/plugins/cache"; do
  ...
  find "$d" -maxdepth 5 -type f -path '*/.claude-plugin/plugin.json' 2>/dev/null | while read -r m; do
    ...
    case "$name" in
      komdosh-dev-*) printf '%s %s %s\n' "$name" "$(jq -r '.version // "?"' "$m")" "$(dirname "$(dirname "$m")")" ;;
    esac
  done
done | sort -u
```

If the scan returns nothing (a non-standard install path), say so and treat every plugin-dependent gate as **UNKNOWN**, not MET — an undetectable plugin is not an absent one.

Map gates to the plugin that provides them, and mark a gate **N/A** only when its plugin is confirmed absent:

| Gate | Required plugin / track |
|---|---|
| Module-boundary scan (#8), Coroutine-safety scan (#9), PII scan, Liquibase immutability (#10), jOOQ freshness (#11) | `komdosh-dev-spring-core` (always installed in practice) |
| Service readiness (#15), Code review (#13) | `komdosh-dev-spring-core` (`code-reviewer`) |
| QA artifacts fresh (#14), security/PII audits, vendor-leak audit | `komdosh-dev-spring-quality` |
| Release readiness (#17), Changelog (#18), Rollback playbook (#19), ABI delta (#20), Publish config (#21) | `komdosh-dev-spring-delivery` (this plugin) |
| Prior-work / rationale lookups feeding gates #1–#3 | `komdosh-dev-revealer` (optional; absence never blocks) |
| Dependency, flake, and load-test gates (none in v1) | `komdosh-dev-kotlin-extras` |
| Infrastructure review gates (none in v1 — infra changes route to `/infra-review`) | `komdosh-dev-infra-core`, `komdosh-dev-infra-iac`, `komdosh-dev-infra-k8s` |

Track also gates the delivery set: rollback playbook (#19) applies only on the **service** track; ABI delta (#20) and publish config (#21) only on the **library** track.

Any installed `komdosh-dev-*` plugin the table doesn't name contributes no gates — report it in the "Plugins detected" line anyway, so a newly-shipped plugin surfaces as an obvious gap in this table rather than vanishing silently.
...
**Gate 13 — Code review.**
- Best-effort: look for a marker file `docs/.last-review.json` (the `code-reviewer` agent could write one when it runs cleanly). If absent or older than HEAD, mark PENDING/UNKNOWN. If present and ≥ HEAD with `blockers: 0`, MET.
- (Plan: `code-reviewer` may need a tiny update to drop a marker — note this as an optional follow-up; for v1 just always recommend running `/review` after code changes.)

**Gate 14 — QA artifacts fresh.**
- N/A if `komdosh-dev-spring-quality` plugin not installed.
...
These gates apply only when `komdosh-dev-spring-delivery` is installed. When the plugin is absent, all five report `N/A`.
