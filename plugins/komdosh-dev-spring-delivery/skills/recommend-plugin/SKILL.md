...
**Primary:** `komdosh-dev-spring-core` → agent `event-consumer-author`
...
- **Installed:** ❌ no — install with `/plugin install komdosh-dev-spring-core@komdosh-claude-developer`
...
**Primary:** `komdosh-dev-spring-delivery` → `/lifecycle status`
...
**Primary:** `komdosh-dev-spring-quality` → `/qa plan`
...
**Alternates (lower confidence):**
- `komdosh-dev-spring-quality` → `/qa postman` — if a Newman-runnable smoke suite is more useful than a checklist.
- `komdosh-dev-spring-quality` → `/qa console` — if a non-CLI teammate needs a clickable HTML tester.
