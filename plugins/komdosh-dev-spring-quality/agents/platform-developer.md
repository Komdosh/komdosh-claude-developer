...
6. ADR draft (via `/adr-new`) for the introduction of `common/` if this is the first abstraction
...
Follow `rules/gradle-build.md` for `settings.gradle.kts` and `common/build.gradle.kts`. The convention plugin (`buildSrc/.../kotlin-service.gradle.kts`) usually needs no change — `common/` follows the same shape as `domain/`. Confirm `common/build.gradle.kts` has NO Spring, Micrometer, jOOQ, Jackson, or Reactor on its classpath.
...
| Need | Agent |
|---|---|
| First-time module creation needs `settings.gradle.kts` + `common/build.gradle.kts` | `rules/gradle-build.md` |
| The ADR for "introduce common/" | `/adr-new` (via `/adr-new`) |
| Test updates beyond mechanical fake substitutions | `test-writer` |
| Full readiness audit after the refactor lands | `code-reviewer` at `scope=service` (via `/service-health`) |
