...
The agent runs the matching sub-skills (`match-routes-to-filters`, `check-error-leakage`, `audit-jwt-rotation`, `core/pii-safety-scan` at `depth=audit`), classifies findings per `rules/security-audit.md`, and writes the consolidated report.
...
- **Auth BLOCKERs**: "Run `core/backend-implementer` (per `core/rules/spring-security.md`) to add the missing rule(s)."
- **Error-leakage BLOCKERs**: "Run `core/cleanuper` to mechanically convert handlers to ProblemDetail; for handlers that need redesign, route to `core/backend-implementer` (per `core/rules/spring-security.md`)."
