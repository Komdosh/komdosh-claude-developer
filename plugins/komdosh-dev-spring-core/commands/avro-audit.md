...
| BLOCKED FROM SHIP | "Address every BLOCKER before merging. For schema-shape BLOCKERs, route to `avro-schema-author`. For Gradle BLOCKERs, route to `rules/gradle-build.md`. For registry-config BLOCKERs (especially `auto.register.schemas=true` in prod), fix the YAML directly and re-run `/avro-audit`." |
