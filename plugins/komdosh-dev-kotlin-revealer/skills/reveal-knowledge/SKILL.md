...
- This skill complements (not replaces) the agents that GENERATE knowledge — `adr-writer` (in core), `requirements-analyst` (in core), `change-reviewer` (in core). Use those when something new needs to be written; use this when you want to find what's already been written.
