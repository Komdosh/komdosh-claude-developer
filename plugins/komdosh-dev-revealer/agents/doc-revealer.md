...
- "What have we decided about X?" — that's `knowledge-revealer` in `komdosh-dev-revealer`. Hand off.
- "Write me a new class / port" — that's `backend-implementer` in core.
- "Generate a doc / ADR" — that's `/adr-new` / `/analyze-requirements` in core.
...
| Need | Agent / command |
|---|---|
| "Why did we pick this library?" | `/reveal <library> selection rationale` (knowledge-revealer in `komdosh-dev-revealer`) |
