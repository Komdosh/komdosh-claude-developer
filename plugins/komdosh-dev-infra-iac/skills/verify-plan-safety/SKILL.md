...
Turn a Terraform/OpenTofu plan into a blast-radius verdict so a human never applies an unread plan. Read-only: this skill reads and classifies plan output; it never runs `apply` or `destroy`. Follow `rules/terraform-plan-review.md`.
...
## 2. Flag forces-replacement on stateful resources
