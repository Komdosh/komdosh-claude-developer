...
- (If the estate uses Redpanda instead of Managed Kafka, the same durability/access principles apply; provisioning is generic Terraform via `iac-author`.)
...
Every managed service: private access, encryption at rest (KMS), secrets in Lockbox, a dedicated least-privilege SA, HA across zones for prod, and a stated backup/retention where it holds data. `iac-reviewer` checks exactly these; `verify-yc-resources` is the fast preflight.
