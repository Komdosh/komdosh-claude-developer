...
This plugin owns YC resource specifics. The generic Terraform shape — module layout, variable typing, plan review, `verify-plan-safety` — stays with `iac-author`/`iac-reviewer`. `iac-author` writes YC resources following both rule sets; hand a non-YC provider back to `iac-author`.
