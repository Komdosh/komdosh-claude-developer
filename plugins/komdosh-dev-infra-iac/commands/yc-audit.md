...
Run a Yandex Cloud security-and-reliability audit with the `iac-reviewer` agent.
...
Output is a BLOCKER/WARNING/INFO report with file:line and the concrete blast radius, plus the single highest-value fix. Read-only — never applies or edits; fixes route to `iac-author`.
