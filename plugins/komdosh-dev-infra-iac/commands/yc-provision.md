...
Invoke the `iac-author` agent to write Yandex Cloud Terraform for `$ARGUMENTS`.
...
It self-checks with `verify-yc-resources`, produces a plan for review, and never runs `apply`. Generic Terraform shape is delegated to `iac-author`. Follow with `/yc-audit` and `/tf-plan-review`.
