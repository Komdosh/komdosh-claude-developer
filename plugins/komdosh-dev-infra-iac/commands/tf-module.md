...
Invoke the `iac-author` agent to write or refactor Terraform/OpenTofu for `$ARGUMENTS`.

- **name/resource** — the first positional argument: a module name (`network`, `postgres`) or a resource to add.
- **cloud** — from `--cloud=`; when `yandex` and the `komdosh-dev-infra-iac` plugin is installed, the author delegates managed-service/IAM/network specifics to `iac-author`.
