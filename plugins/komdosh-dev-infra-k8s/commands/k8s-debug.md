...
The agent gathers read-only evidence via `probe-cluster-state` (describe, previous-container logs, events, top), maps the symptom to its **actual** root cause with an explicit evidence chain, and proposes the smallest manifest change that fixes the cause — routed to `k8s-author` / GitOps to apply.
