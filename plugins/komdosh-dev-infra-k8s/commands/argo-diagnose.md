...
Invoke the `k8s-diagnostician` agent to diagnose `$ARGUMENTS`.
...
It never runs `argocd app sync/set/rollback` or `kubectl edit`, and never chains a manual workaround to hide the symptom. A Degraded app caused by a crashing pod is routed to `k8s-diagnostician` (if the kubernetes plugin is installed). If ArgoCD isn't reachable, it reasons from the git manifests plus your symptoms and says so.
