...
- [`security-auditor`](agents/security-auditor.md) — orchestrates the three sub-audits, classifies findings, writes the consolidated report. Read-only — never modifies code or config; never extracts secrets; never invokes `/upgrade` or `/add-endpoint`. Recommends remediation by handing off to `core/security-expert` (writes new filters) or `core/observability-expert` (adds correlation-id propagation if missing).
