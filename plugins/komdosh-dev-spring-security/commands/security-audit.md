# /security-audit [--scope=auth|error-leakage|jwt|all] [--module=<glob>]

Run the full Spring-specific security audit (or a single category if `--scope` is set). Produces `docs/security/audit-YYYY-MM-DD.md` with findings classified BLOCKER / WARNING / INFO + per-finding remediation.
...
The agent runs the matching sub-skills (`match-routes-to-filters`, `check-error-leakage`, `audit-jwt-rotation`), classifies findings per `rules/security-audit.md`, and writes the consolidated report.
