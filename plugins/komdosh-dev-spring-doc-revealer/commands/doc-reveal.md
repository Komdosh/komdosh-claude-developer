...
- Distinct from `/reveal` (in `komdosh-dev-spring-revealer`): `/reveal` answers "what have we already decided?"; `/doc-reveal` answers "what does this *mean* / how do I use it?". They are complementary; if you're not sure which you want, run `/reveal` first — if it returns "no prior work", come here.
