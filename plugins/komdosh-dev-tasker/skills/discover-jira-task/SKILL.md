...
Called by the `/jira-task` command before invoking the `jira-task-coordinator` agent. Not used by any other command, agent, or skill.
...
Emit the structured bundle (see "Output" above). The `/jira-task` command will pass it to the `jira-task-coordinator` agent.
