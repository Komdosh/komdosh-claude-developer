...
If `--topic` was not supplied, derive it from the event name per [events plugin naming](../../komdosh-dev-spring-events/rules/event-consumers.md#topic--queue-naming): `<aggregate>.<past-tense-verb>.v1` (e.g. `OrderCreated` → `orders.created.v1`).
...
- **Inbound, consumer doesn't exist yet** → "Run `event-consumer-author` (from komdosh-dev-spring-events) to write the consumer for `<topic>`. The DTO is generated; the consumer agent will reference it directly."
