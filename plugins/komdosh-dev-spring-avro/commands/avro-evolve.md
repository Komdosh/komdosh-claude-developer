...
- **Need a Gradle change** (e.g. add a new module for the v2 schema, or pin a new Avro version) → `build-expert`.
- **Producer needs to dual-emit** during the cutover → `backend-implementer`.
- **Consumer needs to read both v1 and v2** → `event-consumer-author` (events plugin). Mention the dual-version pattern from [`event-consumers.md`](../../komdosh-dev-spring-events/rules/event-consumers.md#schema-evolution).
