...
The 16 gates, in execution order. Earlier gates are typically cheaper — fix them first.
...
A gate that does not apply to the current change (e.g., no schema change → migration gate, no qa plugin → QA-artifacts gate) is reported as `N/A` rather than blocking.
