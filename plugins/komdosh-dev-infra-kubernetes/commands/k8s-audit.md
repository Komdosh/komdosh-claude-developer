...
Run a hardening audit with the `k8s-hardening-auditor` agent.
...
Output is a BLOCKER/WARNING/INFO report with file:line and concrete impact. Plaintext Secrets route to `secrets-sentinel`; fixes route to `k8s-manifest-author`. Read-only — never applies or edits.
